...
def plot_comparison(iters, methods: list, costs: list, average_costs: list,
                    stationary_classifier: pd.DataFrame, ax, ax_i, window: int, weights=None,
    ...
    if stationary_classifier is not None:
        ...
        for idx, column in enumerate(stationary_classifier.columns):
            if 'cost' in column and 'c1' not in column:
                ...
                ax.plot(iters, rolled[column], label='Threshold: {}'.format(threshold),
    ...
    for label, cost, average_cost, average_cost_bound in zip(methods, costs, average_costs, average_cost_bounds):
        ...
        if 'Boltzmann' in label:
            ...
            boltz_count += 1
            label = 'Boltzmann Learning'
        else:
            color = bench_colors[label]
            bench_count += 1
        ax.plot(iters, rolling_cost, '-', label='{}'.format(label), color=color)
    freq = 50_000
    new_xticks = timestamps[0::freq].dt.to_period('D')
    ax.set_xticks(new_xticks.index, new_xticks)
    ...
    #         ax.legend(sorted_handles, sorted_labels, fontsize=legend_fs, loc='upper right', bbox_to_anchor=(1.0, 2.2))
    #     except:
...
if __name__ == '__main__':
    ...
    for i, (type1_weight, type2_weight) in enumerate([(1.,1.), (5.,1.)]):
        if unit_step:
            step = '_unit'
        else:
            step = ''
        base_file = f'../data/ember_short{step}_{n_train}'
        classifier_file = base_file + f'{type1_weight}_{type2_weight}_short_classifier.pkl'
        ...
        plot_comparison(iters=x_iters, methods=methods_list,
                        ...
                        window=window_size, ax=ax[i], ax_i=i)
